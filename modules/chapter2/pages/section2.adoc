...
. Enable the MLflow Operator component by patching the DataScienceCluster object.
+
...
. Create an `MLflow` custom resource (CR) as `MLflow.yaml`.
...
----
+
NOTE: This is `Minimal development or test deployment`.
Uses `SQLite` for the backend store and a persistent volume claim (PVC) for file-based artifact storage.

. Deploy the `MLflow` resource to your cluster by running the following command.
+
[source,bash,role=execute,subs="attributes,quotes"]
----
...
+
```
mlflow-5865ff54c7-d9nm6                                           2/2     Running   2          6h18m
mlflow-operator-controller-manager-967bf5d4d-d5vpr                1/1     Running   1          9h
```
In the OpenShift AI dashboard navigation bar, click the Applications menu.
Confirm that the MLflow UI is displayed in the list.

+
image::mlflow.png[role=media-seconds-5]
...
. You can see the experiment in MLflow dashboard.

.. Select your workspace.
+
image::mlflow1.png[role=media-seconds-5]

.. Select `lemonades` from drop down menu.
+
image::mlflow2.png[role=media-seconds-5]

.. Click `my-model-v2-eval` but you will not be able to see the results.
+
image::mlflow3.png[role=media-seconds-5]

.. Here is the example output for `garak` with benchmark as `avid_ethics`.
In this example, you can notice model is different (`Qwen3.6-35B-A3B`).
+
image::mlflow7.png[role=media-seconds-5]
...
+
image::mlflow4.png[role=media-seconds-5]

.. Now you can see the demo experiment results.
+
image::mlflow5.png[role=media-seconds-5]
+
image::mlflow6.png[role=media-seconds-5]
