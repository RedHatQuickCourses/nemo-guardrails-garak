...
Let’s update the ConfigMap to include the regular expression (regex) detection.
It is a built-in, low-latency utility used to block, filter, or validate specific text patterns (such as Social Security Numbers, credit cards, or banned keywords) before they hit the LLM or before a response is delivered to the user.

== Hands-on lab

=== Deployment

. Change the directory to `system-message-plus-regex`.
+
...
. Review the updated ConfigMap (`configmap.yaml`) file which includes regex detection.
+
[tabs]
====
Highlights::
+
.. regex_detection config is added.
.. "regex check input" as input flow is added.
...
. You can notice the built in response for v1/chat/completions endpoint.
+
[source,bash,role=execute,subs="attributes,quotes"]
----
curl -s -X POST "https://$NEMO_ROUTE/v1/chat/completions" \
